      ...
      <div class="glass-card">
        ...
        <div class="table-wrap hidden md:block">
          <table class="ios-table">
            ...
            <tbody>
              <tr v-for="wh in warehouses.data" :key="wh.id" class="table-row">
                <td>
                  <span class="badge" :class="wh.is_active ? 'badge--green' : 'badge--gray'">
                ...
                <td class="text-center">
                  <div class="action-group">
                    ...
                    <button class="action-btn action-btn--red" @click="deleteWarehouse(wh)">Delete</button>
                  </div>
                </td>
              </tr>
              <tr v-if="warehouses.data.length === 0"><td colspan="5" class="empty-cell">No warehouses found.</td></tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="block md:hidden p-4">
          <div class="mobile-list">
            <div v-for="wh in warehouses.data" :key="'m-'+wh.id" class="mobile-card">
              <div class="mobile-card__top">
                <span class="td-mono" style="font-size:0.68rem;">{{ wh.code }}</span>
                <span class="badge" :class="wh.is_active ? 'badge--green' : 'badge--gray'">
                  {{ wh.is_active ? 'Active' : 'Inactive' }}
                </span>
              </div>
              <h3 class="mobile-card__name">{{ wh.name }}</h3>
              <div v-if="wh.address" class="mobile-card__address">
              ...
              <div class="mobile-card__actions">
                <button class="action-btn action-btn--blue action-btn--full" @click="openModal(wh)">Edit</button>
                <button class="action-btn action-btn--red action-btn--full" @click="deleteWarehouse(wh)">Delete</button>
              </div>
            </div>
            <div v-if="warehouses.data.length === 0" class="empty-cell">No warehouses found.</div>
          </div>
        </div>

        <!-- Pagination -->
        <div v-if="warehouses.links.length > 3" class="pagination">
          <template v-for="(link, i) in warehouses.links" :key="i">
    ...
    <Modal :show="showModal" @close="closeModal" maxWidth="md">
      <div class="modal-body">
        <div class="modal-header">
          ...
          <div>
            <h2 class="modal-header__title">{{ isEditing ? 'Edit Warehouse' : 'New Warehouse' }}</h2>
            <p class="modal-header__sub">{{ isEditing ? 'Update warehouse details' : 'Add a new location to the network' }}</p>
          </div>
        ...
        <form @submit.prevent="submitForm" class="modal-form">
          <div class="field">
            ...
            <TextInput id="name" v-model="form.name" type="text" required placeholder="e.g. Main Warehouse Jakarta" />
            <InputError :message="form.errors.name" />
          </div>
          <div class="field">
            <InputLabel for="code" value="Warehouse Code" />
            <TextInput id="code" v-model="form.code" type="text" required placeholder="e.g. JKT-01" style="font-family: var(--font-mono); text-transform: uppercase;" />
...
const openModal = (wh = null) => {
  form.clearErrors();
  if (wh) { isEditing.value = true; editingId.value = wh.id; form.name = wh.name; form.code = wh.code; form.address = wh.address; form.is_active = !!wh.is_active; }
...
const submitForm = () => {
  ...
  isEditing.value ? form.put(route('warehouses.update', editingId.value), opts) : form.post(route('warehouses.store'), opts);
};
...
.mobile-card__address { display: flex; align-items: flex-start; gap: 0.3rem; font-size: 0.78rem; color: rgba(0,0,0,0.40); }
